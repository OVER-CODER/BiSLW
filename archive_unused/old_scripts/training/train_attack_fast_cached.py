...
Uses cache from precompute_all_attacks.py - NO VAE operations during training!
This is ~100x faster than on-the-fly attack application.

Training only involves:
- Latent-space embedding (fast)
- Latent-space extraction (fast)
- Loss computation (fast)
"""
...
class FastAttackTrainer:
    """Ultra-fast trainer using precomputed attacked latents."""
    ...
    def train_step(self, z_orig, w_batch, attacked_latents_dict):
        """
        Training step using precomputed attacked latents.
        
        attacked_latents_dict: dict mapping attack_name -> z_attacked tensor
        """
        B = z_orig.shape[0]
        
        # Randomly select attack for each sample
        attack_indices = np.random.randint(0, len(ATTACK_NAMES), B)
        ...
        # Also get clean embedding prediction (for quality maintenance)
    ...
    def train_epoch(self, cache_data, epoch, batch_size=32):
        """Train one epoch - FAST because no VAE!"""
        ...
        # Create attacked latents dict for this epoch
        ...
        for start in pbar:
            ...
            # Get attacked latents for this batch
    ...
    def evaluate(self, cache_data, n_samples=200):
        """Evaluate on all attacks using cached latents."""
        self._set_eval_mode()
...
def main():
    ...
    parser.add_argument('--cache', type=str, required=True, help='Precomputed attacked latents cache')
    parser.add_argument('--resume', type=str, help='Checkpoint to resume from')
    parser.add_argument('--epochs', type=int, default=100)
    parser.add_argument('--batch-size', type=int, default=64)  # Can use larger batch since no VAE
    ...
    print(f"Loading precomputed cache: {args.cache}")
    ...
    for name in ATTACK_NAMES:
        ...
        if key not in cache_data:
            print(f"ERROR: Cache missing {key}")
            return
    
    n_samples = len(cache_data['z_orig'])
    print(f"Loaded {n_samples} samples with {len(ATTACK_NAMES)} attacks")
    ...
    print("\n" + "="*60)
    print("FAST ATTACK-AWARE TRAINING (Precomputed)")
    print("="*60)
