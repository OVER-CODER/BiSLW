...
def main():
    ...
    with torch.no_grad():
        for i in tqdm(range(0, num_eval, 16), desc="Evaluating"):
            ...
            # LPIPS and SIFID
            lpips_result = metrics.compute_lpips(batch, z_wm)
            lpips_vals.extend(lpips_result.values)
            sifid_result = metrics.compute_sifid(batch, z_wm)
            sifid_vals.extend(sifid_result.values)
            if device.type == 'mps':
    ...
    print(f"\nLPIPS:")
    print(f"  Mean: {np.mean(lpips_vals):.4f} +/- {np.std(lpips_vals):.4f}")
    print(f"\nSIFID:")
    print(f"  Mean: {np.mean(sifid_vals):.4f} +/- {np.std(sifid_vals):.4f}")
